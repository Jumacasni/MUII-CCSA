...
def models_arima():
	...
	model_temperature = pm.auto_arima(df_temperature.head(1000), start_p=1, start_q=1,
	...
	model_humidity = pm.auto_arima(df_humidity.head(1000), start_p=1, start_q=1,
				       test='adf',       # use adftest to find optimal 'd'
				       max_p=3, max_q=3, # maximum p and q
				       m=1,              # frequency of series
				       d=None,           # let model determine 'd'
...
def models_prophet():
	...
	df_temperature = pd.concat(data_temperature.head(1000), axis=1, keys=headers_temperature)
	...
	df_humidity = pd.concat(data_humidity.head(1000), axis=1, keys=headers_humidity)
